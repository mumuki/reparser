...
spec = do
  describe "reparse" $ do
    ...
    it "`'`" $ do
      reparse "'" `shouldBe` Just (SyntaxError 1 1 BrokenSingleQuote)

    it "`\"`" $ do
      reparse "\"" `shouldBe` Just (SyntaxError 1 1 BrokenDoubleQuote)

    it "`\"hello`" $ do
      reparse "\"hello" `shouldBe` Just (SyntaxError 1 1 BrokenDoubleQuote)
